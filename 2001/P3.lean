...
Adapted from Archive/Imo/Imo2001Q3.lean (Jeremy Tan).
...
theorem imo2001_p3 (G B : Fin 21 → Finset ℕ)
    (hG : ∀ i, (G i).card ≤ 6)
    (hB : ∀ j, (B j).card ≤ 6)
    (hcommon : ∀ i j, ∃ p, p ∈ G i ∧ p ∈ B j) :
    ∃ p : ℕ,
      3 ≤ (Finset.univ.filter (fun i : Fin 21 => p ∈ G i)).card ∧
      3 ≤ (Finset.univ.filter (fun j : Fin 21 => p ∈ B j)).card := by
  exact result ⟨hG, hB, fun i j => Finset.not_disjoint_iff.mpr (hcommon i j)⟩
